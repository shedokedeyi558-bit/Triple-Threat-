...
interface QuestionState {
  ...
  options?: string[];
  timer: number; // overall exam timer (seconds) — same value for all questions in this pill
}
...
function ExamTimerBar({ secondsLeft, totalSeconds }: { secondsLeft: number; totalSeconds: number }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      <Clock size={13} style={{ color: isCritical ? "#ef4444" : isWarning ? "var(--accent-amber)" : "var(--text-muted)", flexShrink: 0 }} />
      <div style={{ flex: 1, height: 4, borderRadius: 2, backgroundColor: "#1E1E1E", overflow: "hidden" }}>
        <motion.div
          ...
          style={{
            height: "100%",
            borderRadius: 2,
            backgroundColor: isCritical ? "#ef4444" : isWarning ? "var(--accent-amber)" : "var(--accent-indigo)",
          }}
        />
      </div>
      <span style={{
        fontSize: 12, fontFamily: "monospace", fontWeight: 700, flexShrink: 0, minWidth: 36, textAlign: "right",
...
// ── Inline answer UI (replaces PillPlay — no per-question timer) ───────────
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div>
        ...
        <p style={{ fontSize: 17, fontWeight: 600, lineHeight: 1.5, color: "var(--text-primary)", margin: 0 }}>
          {question}
        </p>
      </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: isSelected ? "1.5px solid var(--accent-amber)" : "1.5px solid var(--border-subtle)",
                  backgroundColor: isSelected ? "rgba(232,163,61,0.08)" : "var(--bg-card)",
          ...
          <button
            ...
            style={{
              ...
              fontSize: 14, fontWeight: 700, cursor: !selected || isLoading ? "not-allowed" : "pointer",
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <button
            ...
            style={{
              ...
              fontSize: 14, fontWeight: 700, cursor: !typed.trim() || isLoading ? "not-allowed" : "pointer",
              display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
              opacity: isLoading ? 0.6 : 1,
            }}
          >
...
export default function VipPlayPage() {
  ...
  // Exam timer — counts down from the admin-set total for this pill
  ...
  const startOrResume = useCallback(async () => {
    ...
    try {
      ...
      // Use the question's timer field as the total exam duration
      // If resuming, we don't know how much time was left — restart from full
      // (backend should enforce time expiry server-side anyway)
      startTimer(res.question.timer > 0 ? res.question.timer : 300);
      setPhase("playing");
    } catch (err) {
    ...
    try {
      ...
      if (res.correct) {
        if (res.streak_complete && res.prize !== undefined) {
          ...
          // Instant advance — no flash delay, exam feel
    ...
    <div className="min-h-screen" style={{ backgroundColor: "#0A0A0A", color: "var(--text-primary)" }}>
      ...
      <header className="sticky top-0 z-40 px-4 py-3 border-b"
        ...
        <div className="max-w-lg mx-auto" style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{
                ...
                backgroundColor: "rgba(232,163,61,0.15)", color: "var(--accent-amber)", boxShadow: "0 0 8px rgba(232,163,61,0.2)",
              }}>
                VIP
              </span>
          ...
          {/* Exam timer bar — only during playing */}
          {phase === "playing" && (
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === "loading" && (
            <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              className="flex flex-col items-center justify-center gap-4" style={{ minHeight: "55vh" }}>
          ...
          {phase === "streak_complete" && (
            <motion.div key="win" initial={{ opacity: 0, scale: 0.88 }} animate={{ opacity: 1, scale: 1 }}
              ...
              <div style={{ display: "flex", gap: 12, width: "100%", maxWidth: 320 }}>
                <button onClick={() => router.push("/wallet")} style={{ flex: 1, padding: "12px 0", borderRadius: 10, border: "none", backgroundColor: "var(--accent-amber)", color: "#000", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                  View Wallet
                </button>
                <button onClick={() => router.push("/play")} style={{ flex: 1, padding: "12px 0", borderRadius: 10, border: "1px solid var(--border-subtle)", backgroundColor: "transparent", color: "var(--text-primary)", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                  Back to Play
                </button>
              </div>
            </motion.div>
          ...
          {phase === "time_up" && (
            <motion.div key="time_up" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
              style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 20, paddingTop: 48, textAlign: "center", padding: "48px 16px 0" }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 13, color: "var(--text-muted)" }}>
                  You reached question {displayQuestionNum} of {totalQuestions} before time ran out.
                </p>
              </div>
              ...
              <div style={{ display: "flex", gap: 12, width: "100%", maxWidth: 320 }}>
                <button onClick={startOrResume} style={{ flex: 1, padding: "12px 0", borderRadius: 10, border: "none", backgroundColor: "var(--accent-indigo)", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                  Try Again — ₦{entryFee.toLocaleString()}
                </button>
                <button onClick={() => router.push("/play")} style={{ flex: 1, padding: "12px 0", borderRadius: 10, border: "1px solid var(--border-subtle)", backgroundColor: "transparent", color: "var(--text-secondary)", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                  Back
                </button>
              </div>
              <p style={{ fontSize: 11, color: "var(--text-muted)", maxWidth: 280 }}>
                Trying again starts a fresh attempt at full entry fee.
              </p>
            </motion.div>
          )}
          ...
          {phase === "error" && (
            <motion.div key="error" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <button onClick={startOrResume} style={{ padding: "10px 24px", borderRadius: 10, border: "none", backgroundColor: "var(--accent-indigo)", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                Retry
              </button>
            </motion.div>
          )}
